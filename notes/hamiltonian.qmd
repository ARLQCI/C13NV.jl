...
date: today
date-format: long
title: 'Generalized Hamiltonian for multiple ¹³C atoms around an NV center'
...
We discuss first the high-level structure of the Hilbert space (@sec-hilbert-structure) and the Hamiltonian/Liouvillian (@sec-hamiltonian-structure) before deriving the operators in detail. @sec-lab lists the lab frame Hamiltonian and defines the spin operators for the electronic and nuclear spins. @sec-rwa transforms the lab frame Hamiltonian into the rotating frame. This yields one possible Hamiltonian to use numerically (`frame = :rwa`). @sec-diag goes further by analytically diagonalizing the hyperfine interaction between the nuclear spin and the electronic spin of the NV center. This yields an alternative form of the Hamiltonian, albeit not one that provides a pathway to a better analytic understanding in the presence of a non-zero magnetic field. Lastly, @sec-dissipation describes the Lindblad operators for the full dissipative model.


## Structure of the Hilbert space {#sec-hilbert-structure}

* The optical Hilbert space is $\HilbertO$ with levels $\ket{G}$, $\ket{E}$, $\ket{M}$. The Hilbert space may be truncated to $\ket{G}$ if the parameter `Λ` representing $\Lambda(t)$ is passed as `nothing`. Note that `Λ = 0.0` is possible to force inclusion of all optical levels.
* The Hilbert space of the NV center electronic spin is $\HilbertS$ with levels $\ket{+1}$, $\ket{0}$, $\ket{-1}$. The $\ket{-1}$ level is truncated if the `Ω`$_{-}$ parameter representing $\Omega_{-}(t)$ is given as `nothing`, and likewise for $\ket{+1}$ and `Ω`$_{+}$. Either `Ω`$_{+}$ or `Ω`$_{-}$ can be passed as `0.0` to force inclusion of the level.
...
where the factor $\frac{1}{\sqrt{2}}$ is to compensate for the normalization factor in the $\hat{S}_x$ operator, see below in @sec-lab. In a two-color rotating frame for the central frequencies $\omega_{\pm}$, this results in four independent control fields:
...
3. If the full optical Hilbert space $\HilbertO$ is required (`Λ` is given), extend each operator into the full $\Hilbert$ according to
   ...
   cf. the first line of @eq-Hilbert-space-structure, with the trivial $𝟙_G =  𝟙_E  = 𝟙_M = 1$. That is, we are simply padding the Hamiltonian with zeros to reach the size of the full Hilbert space. The choice of $𝟘_S$ and $𝟘_I$ reflects the fact that there are no coherent dynamics in the $\ket{E}$ and $\ket{M}$ manifolds.


The `make_nv_system` function returns a Liouvillian if `Λ` is given, or if decay rates $\gamma_{\pm 1}$ for the spontaneous decay in the electronic spin levels is given with a decay rate $> 0$. The incoherent drive with `Λ` is time-dependent, and must be set up specially, see @sec-dissipation for details.
...
For $\theta = \phi = 0$, this is $\hat{B}_I^{(n)} = B \hat{I}_z^{(n)}$. Going through all the terms in @eq-lab-hamiltonian, we thus find
...
\hat{H}_{\omega_{+}, \RWA} & = -\ketbra{+1}{+1} \; \otimes \; 𝟙_I \\
\hat{H}_{\Omega_{-}, \RWA} & = \frac{\mu}{2} \left( \ketbra{0}{-1} + \ketbra{-1}{0} \right) \otimes 𝟙_I \\
\hat{H}_{\Omega_{+}, \RWA} & = \frac{\mu}{2} \left( \ketbra{0}{+1} + \ketbra{+1}{0} \right) \otimes 𝟙_I
\end{split}
...
## Aside: Diagonalizing the Hyperfine Interaction {#sec-diag}
...
$$
\hat{A}_I^{(n)} = A^{(n)} \, \hat{R}^{(n)} \hat{I}_z^{(n)} \hat{R}^{(n)\dagger}
...
There is probably a more elegant way of expressing this transformation in terms of Euler angles.

For the total Hamiltonian, we can now define a "diagonal" (with caveats, see below) frame via the unitary
...
& = (\hat{\delta} - \hat{\omega}(t)) \otimes 𝟙_I + \sum_{n=1}^{N} \left(A^{(n)} \hat{S}_z \otimes \hat{I}_z^{(n)} - \gamma_c 𝟙_S \otimes \hat{R}^\dagger \hat{B}_I^{(n)} \hat{R} \right) + \hat{\Omega}(t) \otimes 𝟙_I\,,
\end{split}
...
While the first term under the sum has been diagonalized by the transformation,  $\hat{R}^\dagger \hat{B}_I^{(n)} \hat{R}$ has been _un_-diagonalized! This severely limits the usefulness of the transformation.
